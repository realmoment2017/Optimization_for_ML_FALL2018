...
def cyclic_coordinate_descent_primal_subgradient(init, steps, imgs, labels, t_bias, proj=lambda x: x):
    ...
    for i, _lambda in enumerate(steps):        
        ...
        for j in range(0, num_features):
            w_ = xs[-1].copy()
#            ftr_idx = np.random.choice(num_features,1, replace=False)  
            ftr_idx = j
            grad_ftr = 0
